...
def unmold_mask(bbox, mask, image_shape,flag_flip_images):
...
#     print(crop.shape, mask.shape)
    minimask = np.zeros((y2 - y1 + 1, x2 - x1 +  1), dtype=bool)
...
def samples_MatrixNetAnchors(db, k_ind, data_aug, debug):
    data_rng   = system_configs.data_rng
    batch_size = system_configs.batch_size
    nclasses   = db.configs["categories"]
    input_size   = db.configs["input_size"]
...
#     mask_detections_batch     = np.zeros((batch_size,200, 7), dtype=np.float32) 
    ...
    seg_tags = [np.zeros((batch_size, max_tag_len), dtype=np.int64) for output_size in output_sizes]
    seg_tag_masks   = [np.zeros((batch_size, max_tag_len), dtype=bool) for output_size in output_sizes]
    seg_labels   = [np.zeros((batch_size, max_tag_len), dtype=np.int32) for output_size in output_sizes]
    seg_tag_lens = [np.zeros((batch_size, ), dtype=np.int32) for output_size in output_sizes]
...
#             print("after", detections.shape, segmentations.shape)
...
#                     dets.append(det_mask_roi + [0])
#                     segs.append(mini_seg_mask)
                ...
                if np.any(mini_seg_mask): # non zero segmentation
                    ...
                    seg_tag_lens[olayer_idx][b_ind] += 1
                else:
                    print('seg mask is empty .. skipping!')
...
#         for det, seg in zip(dets, segs):
#             seg_for_display.append(unmold_mask(det[0:4], seg, (640,640),False))

#         seg_for_display = unmold_mask(np.array(dets)[:,0:4], np.array(segs), (640,640),False)
        
#         display_instances(255 * image, np.array(dets), np.array(seg_for_display).transpose((1, 2,0)), np.zeros((len(dets),)), "./imgs/", str(db_ind)+"_reformatted.jpg")
    ...
    for b_ind in range(batch_size):
        for olayer_idx in range(len(tag_lens)):
            ...
            seg_tag_len = seg_tag_lens[olayer_idx][b_ind]
            seg_tag_masks[olayer_idx][b_ind, :seg_tag_len] = 1    
