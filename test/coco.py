...
from external.nms import soft_nms, soft_nms_merge,nms
...
def mask_merges(m,b, merges, threshold=None):
    m = m.copy()
    for ind in merges:
        base_box = b[ind]
        x1,y1,x2,y2,sc = base_box 
        base_mask = m[ind]*sc
        running_mask = np.ones(base_mask.shape) * sc
#         running_mask[int(y1):int(y2)+1, int(x1):int(x2)+1] *=sc
        for reject in merges[ind]:
            base_mask,running_mask = merge_one_mask(base_mask, base_box, m[reject], b[reject],running_mask)
        m[ind] = base_mask/running_mask
    #pdb.set_trace()
    if threshold:
        m = m > threshold
        m  = m.astype(bool)
...
def merge_one_mask(base_mask, base_box, m, b, running_mask):
    base_mask = base_mask.copy()
    running_mask = running_mask.copy()
    x1b,y1b,x2b,y2b,scoreb = base_box

    x1,y1,x2,y2,score = b
    #print(scoreb, score)
    x1o = int(max(x1b,x1))
    x2o = int(min(x2b,x2))+1
    y1o = int(max(y1b,y1))
    y2o = int(min(y2b,y2))+1
    
#     print(x1o, x2o, y1o, y2o)
    
    base_mask[y1o:y2o, x1o:x2o] = base_mask[y1o:y2o, x1o:x2o]+ score*m[y1o:y2o, x1o:x2o] 
    running_mask[y1o:y2o, x1o:x2o] += score
    return base_mask,running_mask
...
def unmold_mask(bbox, mask, image_shape, threshold=None):
...
#     mask = mask.data.cpu().numpy()
    full_masks = np.zeros((mask.shape[0],)+(h,w), dtype=float)
    for i in range(mask.shape[0]):
        m = mask[i,:, :]
        x1, y1, x2, y2 = bbox[i][:4].astype(int)
        widths = x2-x1+1
        heights = y2-y1+1
        m = cv2.resize(m, (widths, heights), interpolation = cv2.INTER_LANCZOS4)           
        full_masks[i][y1:y2+1, x1:x2+1] = m
    if threshold:
        full_masks = full_masks > threshold
        full_masks = full_masks.astype(bool)
...
def kp_decode(nnet, images, K, matching_threshold=0.5, kernel=3, layers_range = None, output_kernel_size = None,output_sizes=None, input_size = None, base_layer_range = None):
    detections,masks = nnet.test([[images]], dist_threshold=matching_threshold, K=K, kernel=kernel,
...
def test_MatrixNetAnchors(db, nnet, result_dir, debug=False, decode_func=kp_decode):
    ...
    if db.split != "trainval":
        db_inds = db.db_inds[:200] if debug else db.db_inds[:200]
    else:
        db_inds = db.db_inds[:200] if debug else db.db_inds[:200]
    num_images = db_inds.size
    ...
    matching_threshold  = db.configs["matching_threshold"]
    nms_kernel    = db.configs["nms_kernel"]
    flag_flip_images=False#db.configs["test_flip_images"]
    max_dim = db.configs["test_image_max_dim"]
    ...
    for ind in tqdm(range(0, num_images), ncols=80, desc="locating kps"):
        ...
        for scale in scales:
            ...
            resized_image = normalizer(resized_image)
           ...
           # print(dets.shape)
            ...
            if flag_flip_images:
                ...
                dets[1, :, [0, 2]] = out_width - dets[1, :, [2, 0]]
                #dets   = dets.reshape(1, -1, 8)
                masks[1] = np.fliplr(masks[1].transpose((1,2,0))).transpose((2,0,1))
            _rescale_dets(dets, ratios, borders, sizes)
            dets[:, :, 0:4] /= scale
            #detections.append(dets)
            #segmemtations.append(masks)         
    
        detections = np.concatenate(dets, axis=0)
        masks =  np.concatenate(masks, axis=0)
        classes    = detections[..., -1]
        #print(detections.shape, classes.shape)
        #masks = segmemtations      
        # reject detections with negative scores
        ...
        for j in range(categories):
            ...
            keep_inds = (classes == j)
            b_cls = detections[keep_inds][:, 0:5].astype(np.float32).copy()
            m_cls = masks[keep_inds].copy()
#             G(top_bboxes[image_id][j + 1].shape)
...
#             top_bboxes[image_id][j + 1] = detections[keep_inds][:, 0:7].astype(np.float32)
            ...
            nms_keeps,merges = nms(b_cls, 0.6)
            
#             print('here we go!')
#             print(nms_keeps, merges)
            #b = b_cls[nms_keep]
            #m = m_cls[nms_keep]
            #top_bboxes[image_id][j + 1] = b 
            m = unmold_mask(b_cls, m_cls , [height,width]) # ,threshold = 0.5)
            m = mask_merges(m, b_cls, merges,  threshold = 0.5)
            top_bboxes[image_id][j + 1] = b_cls[nms_keeps]
            top_masks[image_id][j + 1]  = m[nms_keeps]
...
#             print(debug_dir)
            ...
            #print(debug_file)
#             cv2.imwrite(debug_file,image)
    print(sum(pre)/len(db_inds), sum(dec)/len(db_inds), sum(post)/len(db_inds), len(db_inds))
    result_json = os.path.join(result_dir, "results.json")
