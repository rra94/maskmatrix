...
class MaskLayer_onedeconv(nn.Module):
    def __init__(self, depth, num_classes):
        super(MaskLayer_onedeconv, self).__init__()
        self.depth = depth
#         self.pool_size = pool_size
        self.num_classes = num_classes
#         self.padding = SamePad2d(kernel_size=3, stride=1)
...
#         self.Xmask = MaskLayer( 256, classes)
        self.Xmask = MaskLayer_onedeconv( 256, classes)
...
class MatrixNetAnchorsLoss(nn.Module):
    ...
    def forward(self, outs, targets):
        ...
        for i in range(len(anchors_heats)):
            #pdb.set_trace()
            #keeps = anchors_heats[i] != -100
            #anchors_heats[i] = anchors_heats[i][keeps]
            #gt_anchors_heat[i] = gt_anchors_heat[keeps]
...
#             print( gt_mask[i].shape)
        ...
        if  pred_masks != None:
            ...
            gt_outline_all_layers = [None for i in range(len(anchors_heats))]
            ...
            for i in range(len(anchors_heats)):
                if torch.sum(gt_seg_tag_masks[i]>0) :
                    ...
                    target_outlines = gt_seg_outlines[i].view(-1,gt_seg_outlines[i].shape[2],gt_seg_outlines[i].shape[3])
                    gt_masks_all_layers[i] = target_masks[positive_ix]
                    gt_outline_all_layers [i] =  target_outlines[positive_ix]
            ...
            gt_outline_all_layers = torch.cat([i for i in gt_outline_all_layers if i != None], dim = 0)
#             save_image(gt_outline_all_layers.float().unsqueeze(1),  "./imgs/target+" + str(i) + "_outline.jpg",5)
#             save_image(gt_masks_all_layers.float().unsqueeze(1),  "./imgs/target+" + str(i) + "_after.jpg",5)
#             time.sleep(5)
#             gt_outline_all_layers = gt_outline_all_layers + 1 
#             gt_masks_all_layers = gt_masks_all_layers * gt_outline_all_layers / torch.sum(torch.sum(gt_outline_all_layers, 1),1).unsqueeze(1).unsqueeze(2).expand_as(gt_outline_all_layers) #attention on outlines
#             pdb.set_trace() 
...
#            mask_loss += F.binary_cross_entropy(pred_masks, gt_masks_all_layers)
           ...
           #edge attentin
            ...
            mask_loss = F.binary_cross_entropy(pred_masks, gt_masks_all_layers, reduce=False) * gt_outline_all_layers

            mask_loss = mask_loss / mask_loss.shape[0]
...
#           
        ...
        loss = (focal_loss + corner_regr_loss) + 8 *  mask_loss
...
# #             scores, inds =torch.topk(scores, min(num_dets, scores.shape[0])) #scores.sort(descending=True)
#             scores, inds = scores.sort(descending=True)
#             num_dets =  min(num_dets, scores.shape[0])
...
#             clses  = _gather_feat(clses, inds).float()
            clses = anchors_clses.view(-1,1).float()
        ...
        dets = torch.cat(layer_dets, dim = 0)
...
#         _, mask_inds = torch.topk(dets[:, 4], 500) #dets[:,4].sort(descending=True)
        _, mask_inds = dets[:,4].sort(descending=True)
        mask_inds = mask_inds[:300]
...
#         _, mask_inds = torch.topk(mask_bboxes[:, -1], mask_bboxes.size(0)) #mask_bboxes[:, -1].sort(descending=True)
        _, mask_inds = mask_bboxes[:, -1].sort(descending=True)
        mask_bboxes = _gather_feat(mask_bboxes, mask_inds)
        dets = _gather_feat(dets, mask_inds)
        boxes_without_scaling = _gather_feat(boxes_without_scaling, mask_inds)
        detections_batch.append(dets)
...
#         norm_boxes = (36. - 1.) / (9. - 1.) * boxes_without_scaling[:,:4] # multiplying by the box upsampling ratios (2 deconvs)
        ...
        norm_boxes = (26. - 1.) / (13. - 1.) * boxes_without_scaling[:,:4] # multiplying by the box upsampling ratios (2 deconvs)
...
#         corners_tl_regr = corners_tl_regrs[i] #[b_ind]
#         corners_br_regr = corners_br_regrs[i] #[b_ind]
#         feature = features[i] #[b_ind]

# #             print(anchors_heat.shape, corners_tl_regr.shape)

#         _ , cat, height, width = anchors_heat.size()
#         height_scale = height_0 / height
#         width_scale = width_0 / width

#         anchors_scores, anchors_inds, anchors_clses, anchors_ys, anchors_xs = _topk(anchors_heat, K)
# #         print(anchors_ys.shape, anchors_xs.shape, anchors_heat.shape)
...
#         bboxes = torch.cat((tl_xs, tl_ys, br_xs, br_ys), dim=2)
...
#         mask_bboxes =  torch.cat((mask_x1, mask_y1, mask_x2, mask_y2), dim=2)

#         clses = anchors_clses.unsqueeze(2).float()

# #             boxes_without_scaling =  torch.cat([bboxes, scores,scores,scores, clses], dim=2)


# #         print(mask_bboxes[0].shape, feature.shape)

#         if (anchors_heat.shape[0] == 1):
#             in_masks = roipool(feature, [mask_bboxes[0].float()], (2 * max_y + 1, 2 * max_x + 1))
#             masks_feats.append(in_masks.unsqueeze(0))
#         else:
#             in_masks = roipool(feature, [mask_bboxes[0].float(), mask_bboxes[1].float()], (2 * max_y + 1, 2 * max_x + 1))
# #             print(in_masks.shape)
#             in_masks = in_masks.view(2, K, in_masks.shape[1], in_masks.shape[2], in_masks.shape[3])
#             masks_feats.append(in_masks)
# #                 masks_feats_flipped.append(in_masks[K:])

#         layer_norm_boxes = bboxes.clone()
#         layer_norm_boxes[:,:,0:2] -= mask_bboxes[:,:,0:2]
#         layer_norm_boxes[:,:,2:4] -= mask_bboxes[:,:,0:2]
# #         norm_boxes = (36. - 1.) / (9. - 1.) * boxes_without_scaling[:,:4] # multiplying by the box upsampling ratios (2 deconvs)
#         layer_norm_boxes = (26. - 1.) / (13. - 1.) * layer_norm_boxes
...
#         dets_in_layer = torch.cat([bboxes, scores,scores,scores, clses], dim=2)
#         layer_dets.append(dets_in_layer)
#         norm_boxes.append(layer_norm_boxes)

#     dets = torch.cat(layer_dets, dim = 1)
#     masks_feats = torch.cat(masks_feats, dim = 1)
#     norm_boxes = torch.cat(norm_boxes, dim = 1)
    
#     print(dets.shape)
#     print(masks_feats.shape)

                    
#     detections_batch = []
#     predmask_batch = []
#     for b in range(batch):
#         all_pred_masks = Xmask(masks_feats[b])
#         target_classes = dets[b, :, -1]
...
#         all_pred_masks = crop_and_resize (all_pred_masks, norm_boxes[b] ,h)                                     
#     #         save_image(all_pred_masks.float().unsqueeze(1),  "./imgs/target+" + str(i) + "ac_predicted.jpg",5)
#         predmask_batch.append(all_pred_masks)
#         detections_batch.append(dets[b])
